...
bool Cherry::init()
{
	...
	std::string ccbiName = "boss_cherry.ccbi";
...
void Cherry::normalMoving(float dt)
{
...
void Cherry::startAnimationNoDirection()
{
	...
	if(m_state != CUMBERSTATENODIRECTION)
	{
		...
		schedule(schedule_selector(Cherry::animationNoDirection));
	}
}

void Cherry::damageReaction(float)
{
	...
	else
	{
		...
		unschedule(schedule_selector(Cherry::damageReaction));
		mAnimationManager->runAnimationsForSequenceNamed("Default Timeline");
	}
}


void Cherry::animationNoDirection(float dt)
{
...
void Cherry::cumberAttack(float dt)
{
...
COLLISION_CODE Cherry::crashWithX(IntPoint check_position)
{
...
COLLISION_CODE Cherry::crashLooper(const set<IntPoint>& v, IntPoint* cp)
{
	for(const auto& i : v)
	{
...
void Cherry::startInvisible()
{
	...
	if(m_invisible.startInvisibleScheduler == false)
	{
		...
		schedule(schedule_selector(Cherry::invisibling));
		m_invisible.startInvisibleScheduler = true;
	}
}

void Cherry::invisibling(float dt)
{
	...
	else
	{
		...
		if(m_invisible.invisibleValue == 255)
		{
			...
			unschedule(schedule_selector(Cherry::invisibling));
		}
	}
	
}

void Cherry::getRandomPosition(IntPoint* ip, bool* finded)
{
...
void Cherry::crashMapForPosition(CCPoint targetPt)
{
...
void Cherry::furyModeScheduler(float dt)
{
