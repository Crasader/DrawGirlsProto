...
void AttendancePopup::myInit(int t_touch_priority, function<void()> t_end_func)
{
	...
	CCSprite* attendance_img = CCSprite::create("attendance_temp.png");
	attendance_img->setPosition(ccp(0,0));
	m_container->addChild(attendance_img);
	...
	addChild(KSGradualValue<int>::create(0, 255, 0.25f, [=](int t){KS::setOpacity(m_container, t);}, [=](int t)
										 {
											 ...
											 add_suction = TouchSuctionLayer::create(touch_priority-1);
											 addChild(add_suction);
											 
											 add_suction->target_touch_began = this;
											 add_suction->delegate_touch_began = callfunc_selector(AttendancePopup::closeAction);
											 
											 add_suction->setTouchEnabled(true);
										 }));
}
