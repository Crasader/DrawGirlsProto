...
private:
	...
	void myInit(int t_touch_priority, function<void()> t_end_func, function<void()> t_upgrade_func)
	{
		...
		KSLabelTTF* title_label = KSLabelTTF::create("미사일 업그레이드", mySGD->getFont().c_str(), 15);
		title_label->setColor(ccRED);
		title_label->setPosition(ccp(0,100));
		m_container->addChild(title_label);
		
		KSLabelTTF* sub_label = KSLabelTTF::create("레벨을 올리면 보스를 좀 더 쉽게 물리칠 수 있어요!", mySGD->getFont().c_str(), 11);
		...
		upgrade_label = KSLabelTTF::create(CCString::createWithFormat("레벨 %d 업그레이드", missile_level+1)->getCString(), mySGD->getFont().c_str(), 12);
