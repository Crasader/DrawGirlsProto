...
CCScene* BugTestScene1::scene()
{
	...
	BugTestScene1 *layer = BugTestScene1::create();
...
//	CCNode* p = reader->readNodeGraphFromFile((ccbiName + ".ccbi").c_str(), this);// readNodeGraphFromFileForFullPath((mySIL->getDocumentPath()+ccbiname2+".ccbi").c_str(), this);
//	//		auto ret = KS::loadCCBIForFullPath<CircleBossCCB*>(this, (mySIL->getDocumentPath()+ccbiname2+".ccbi"));
//	
//	CCSprite* bee_sprite = dynamic_cast<CircleBossCCB*>(p);//ret.first;
