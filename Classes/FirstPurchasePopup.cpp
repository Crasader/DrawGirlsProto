...
//			mySGD->addChangeGoods(kGoodsType_ruby, -mySGD->getRankUpRubyFee(), "승급");
            ...
            if(!t_info.empty())
			{
				...
				fiverocks::FiveRocksBridge::trackEvent("GetGem", "Get_Purchase", "ShopPurchaseFirstGem", ccsf("Stage %04d", mySGD->getUserdataHighPiece()));
...
void FirstPurchasePopup::requestItemDelivery()
{
	...
	command_list.push_back(CommandParam("requestItemDelivery", request_param, json_selector(this, FirstPurchasePopup::resultSaveUserData)));
