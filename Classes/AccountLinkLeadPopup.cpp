...
AccountLinkLeadPopup* AccountLinkLeadPopup::create(int t_touch_priority, function<void()> t_end_func, function<void()> t_action_func)
{
	AccountLinkLeadPopup* t_mup = new AccountLinkLeadPopup();
	t_mup->myInit(t_touch_priority, t_end_func, t_action_func);
	t_mup->autorelease();
	return t_mup;
}

void AccountLinkLeadPopup::myInit(int t_touch_priority, function<void()> t_end_func, function<void()> t_action_func)
{
	...
	KSLabelTTF* title_label = KSLabelTTF::create(myLoc->getLocalForKey(LK::kMyLocalKey_accountLinkLeadTitle), mySGD->getFont().c_str(), 12);
	...
	StyledLabelTTF* slttf = StyledLabelTTF::create(myLoc->getLocalForKey(LK::kMyLocalKey_accountLinkLeadContent), mySGD->getFont().c_str(), 13, 999, StyledAlignment::kCenterAlignment);
	...
	KSLabelTTF* action_label = KSLabelTTF::create(myLoc->getLocalForKey(LK::kMyLocalKey_accountLinkLeadButton), mySGD->getFont().c_str(), 13);
	...
	action_button->addTargetWithActionForControlEvents(this, cccontrol_selector(AccountLinkLeadPopup::actionAction), CCControlEventTouchUpInside);
...
void AccountLinkLeadPopup::closeAction(CCObject* sender, CCControlEvent t_event)
{
	if(!is_menu_enable)
		return;
...
void AccountLinkLeadPopup::actionAction(CCObject* sender, CCControlEvent t_event)
{
	if(!is_menu_enable)
		return;
