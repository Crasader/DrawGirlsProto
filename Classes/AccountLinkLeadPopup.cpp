...
void AccountLinkLeadPopup::myInit(int t_touch_priority, function<void()> t_end_func, function<void()> t_action_func)
{
	...
	KSLabelTTF* title_label = KSLabelTTF::create(myLoc->getLocalForKey(kMyLocalKey_accountLinkLeadTitle), mySGD->getFont().c_str(), 12);
	...
	StyledLabelTTF* slttf = StyledLabelTTF::create(myLoc->getLocalForKey(kMyLocalKey_accountLinkLeadContent), mySGD->getFont().c_str(), 13, 999, StyledAlignment::kCenterAlignment);
	...
	KSLabelTTF* action_label = KSLabelTTF::create(myLoc->getLocalForKey(kMyLocalKey_accountLinkLeadButton), mySGD->getFont().c_str(), 13);
