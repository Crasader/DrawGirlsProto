...
#ifndef DrawingJack_JackMissile_h
#define DrawingJack_JackMissile_h
			...
			else if(ingFrame == 20 + 10)
			{
				...
				if(targetNode == myGD->getCommunicationNode("CP_getMainCumberPointer"))
				{
					if(myGD->getCommunication("CP_getMainCumberSheild") == 0)
					{
						...
						myGD->communication("CP_startSpringMainCumber", -shootImg->getRotation());
					}
				}
	...
	void moving()
	{
		...
		if(isEnable)
		{
			...
			if(distance <= 1)
			{
				...
				ccColor4F myColor;
				if(my_type == kMyElementalNonElemental)			myColor = ccc4f(1.f, 1.f, 1.f, 1.f);
				...
				if(targetNode == myGD->getCommunicationNode("CP_getMainCumberPointer"))
				{
					...
					{
						myGD->communication("UI_subBossLife", damage);
						myGD->communication("CP_startSpringMainCumber", directionAngle);
					}
				}
