...
public:
	...
	//	bool ccTouchBegan(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent);
	virtual bool init(KSAlertView* av)
	{
		CCLayer::init();
				 ...
				 if(m_state == SceneState::kStopHat)
				 {
					 ...
					 this->addChild(KSGradualValue<CCPoint>::create
													(m_selectedHat->getPosition(),
													 ccp(m_parent->getViewSize().width / 2.f, m_parent->getViewSize().height / 2.f) +
													 ccp(0, 50),
													 0.8f,
													 [=](CCPoint pt)
													 {
...
//																									 KS::KSLog("kind % value %", (int)i.first->m_reward->m_kind, );
																									 ...
																									 hatBack->setPosition(ccp(m_parent->getViewSize().width / 2.f, m_parent->getViewSize().height / 2.f) +
																																				ccp(0, 50));
...
//		for(auto i :)
	...
	{
		float r = 100.f;
		return ccp(m_parent->getViewSize().width / 2.f, m_parent->getViewSize().height / 2.f) +
							 ccp(r * cos(t), r * 3.f/4.f * sin(t)) + ccp(0, 30);
	}
