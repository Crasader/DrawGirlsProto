...
private:
	...
	void myInit(int stage, CCObject* t_worldmap, SEL_CallFunc d_cancel)
	{
		...
		if(clr_condition == kCLEAR_default)
		{
			...
			stage_number->setColor(ccBLACK);
			stage_number->setPosition(ccp(175,228));
		...
		{
			CCSprite* sspl_back = CCSprite::create("sspl_back_condition.png");
			sspl_back->setPosition(ccp(240,160));
			addChild(sspl_back, kSSPL_Z_back);
			...
			clr_cdt_img->setPosition(ccp(163, 175));
			...
			content->setPosition(ccp(163,110));
			...
			start_menu = CCMenu::createWithItem(start_item);
			start_menu->setPosition(ccp(240,65));
			addChild(start_menu, kSSPL_Z_menu);
