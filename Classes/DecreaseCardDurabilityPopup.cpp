//
//  DecreaseCardDurabilityPopup.cpp
//  DGproto
//
...
RemoveCardSprite* RemoveCardSprite::create( CCTexture2D* t_texture )
{
...
void RemoveCardSprite::startSlowRemoveCard()
{
	stopAllActions();
...
void RemoveCardSprite::startFastRemoveCard()
{
	stopAllActions();
	unschedule(schedule_selector(RemoveCardSprite::slowRemovingCard));
	unschedule(schedule_selector(RemoveCardSprite::fastRemovingCard));
...
void RemoveCardSprite::startRecovery()
{
	stopAllActions();
	unschedule(schedule_selector(RemoveCardSprite::slowRemovingCard));
	unschedule(schedule_selector(RemoveCardSprite::fastRemovingCard));
...
void RemoveCardSprite::recoveringCard()
{
	ing_frame++;
...
void RemoveCardSprite::slowRemovingCard()
{
	ing_frame++;
...
void RemoveCardSprite::fastRemovingCard()
{
	ing_frame++;
...
void RemoveCardSprite::myInit( CCTexture2D* t_texture )
{
...
void DecreaseCardDurabilityPopup::openingAction()
{
...
void DecreaseCardDurabilityPopup::startEffect()
{
...
void DecreaseCardDurabilityPopup::removeParticle()
{
...
void DecreaseCardDurabilityPopup::touchOn()
{
	int durability = myDSH->getIntegerForKey(kDSH_Key_cardDurability_int1, NSDS_GI(stage_number, kSDS_SI_level_int1_card_i, grade_number));
...
void DecreaseCardDurabilityPopup::fastRemoveAction()
{
	remove_particle->stopAllActions();
...
bool DecreaseCardDurabilityPopup::ccTouchBegan( CCTouch *pTouch, CCEvent *pEvent )
{
	if(recovery_menu)
	{
...
void DecreaseCardDurabilityPopup::ccTouchMoved( CCTouch *pTouch, CCEvent *pEvent )
{
	if(is_menu_on)
		recovery_menu->ccTouchMoved(pTouch, pEvent);
}

void DecreaseCardDurabilityPopup::ccTouchEnded( CCTouch *pTouch, CCEvent *pEvent )
{
	if(is_menu_on)
		recovery_menu->ccTouchEnded(pTouch, pEvent);
}

void DecreaseCardDurabilityPopup::ccTouchCancelled( CCTouch *pTouch, CCEvent *pEvent )
{
	if(is_menu_on)
		recovery_menu->ccTouchCancelled(pTouch, pEvent);
}

void DecreaseCardDurabilityPopup::registerWithTouchDispatcher()
{
