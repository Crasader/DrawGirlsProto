...
서버설정 - TEST 파일! , 여기서 설정후 운영툴의 DB서버구성도 를 확인, 제대로 설정되었다 싶으면 이내용을 config.php 에 적용하면 됩니다.
...
****************************************************************************************/
	...
	//슬레이브DB(읽기용) 지정        db명         서버        * 현재는 따로 슬레이브서버가 없으므로 그냥 마스터db와 똑같은db를 지정. = 읽기/쓰기 모두 server1-drawgirls db 사용
	$mainSlave02 = new DBSlave("drawgirls2",$server1);

	//마스터디비0에 슬레이브0 추가  (마스터0-슬레이브0 리플리케이션 설정되어있어야함. 혹은 마스터0=슬레이브0)
	$mainMaster01->addSlave($mainSlave02);
