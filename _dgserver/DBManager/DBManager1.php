...
class CommitManager{
	...
	static public function setSuccess($memberID,$success,$log=""){
		if(!$success)LogManager::addLog("setSuccess fail [".$log."] :".mysql_error());
...
class UserData extends DBTable2{
	...
	public static function selectWithLQForm($param){
		...
		$pvpInfo = new EndlessRank($user->memberID,$weekNo);
		$r["pvpRank"]=$pvpInfo->getMyRank();

		$weeklyInfo = new WeeklyScore($user->memberID,$weekNo);
		$r["weeklyRank"]=$weeklyInfo->getMyRank();
	...
	public static function updateWithLQForm($param){
		...
		if(array_key_exists("level",$param["data"])){
			self::updateWithLQTable($param);
		...
		if(array_key_exists("selectedCharNO",$param["data"])){
			self::updateWithLQTable($param);
		}
...
class Piece extends DBTable2{
	...
	public function __construct($no=null,$order=null){
		...
		self::setLQTableSelectQueryCustomFunction(function ($param){
			...
			if($param["where"]["where"]){
				return $param["where"]["where"];
			}
...
class UserStorage extends DBTable2{
	...
	public function __construct($memberID=null){
		...
		self::setLQTableSelectCustomFunction(function($rData){
			$userdata = UserData::create($rData["memberID"]);
			$rData["nick"]=$userdata->nick;
			return $rData;
...
class WeeklyScore extends DBTable2{
	...
	static public function construct(){
		self::setPrimarykey("no",true);
		self::setDBGroup("log");
		self::setDBTable("WeeklyScore_".TimeManager::getCurrentWeekNoForRank());
		self::setShardKey("memberID");
		...
		self::setLQTableSelectCustomFunction(function($data){
			...
			//$data["rank"]=WeeklyScore::getRank($data["score"]);
			return $data;
		});
	}
	...
	static public function getRank($score){
		$alluser=0;
		$cnt=0;
		while($result = WeeklyScore::getQueryResult("select count(*) from ".self::getDBTable()." where score>".$score)){		// regWeek=".$this->regWeek." and	
			$cnt++;
			if($cnt>50){
				//LogManager::addLog("superBreak getAllUser");
				break;
...
class Character extends DBTable2{
	...
	public static function getExpInfoByExp($exp=0){
		$cs = new CommonSetting("levelExpTable");
		$levelTable =& $cs->getRef("value");
		$lev=0;
		...
		if($lev==0 || $lev>=count($levelTable)){
			$lev = count($levelTable)-1;
		}

		$prevLevel = $lev-1;
		if($pervLevel<0)$prevLevel=0;
		$nextLevel=$lev+1;
		if($nextLevel>count($levelTable))$nextLevel = count($levelTable);
		
		if($levelTable[$lev]==0){
			$r["isMaxLevel"]=true;
			$exp = $levelTable[$lev-1];
		}

		$r["level"]=$lev;
		$r["exp"]=$exp;
		$r["nextLevelExp"]=$levelTable[$lev];
		$r["currentLevelExp"]=$levelTable[$prevLevel];
		$r["nextLevelUpExp"]=$levelTable[$nextLevel];

		return $r;


	}
	public static function getPowerInfo($level){
		$cs = new CommonSetting("levelPrice");
		$mInfo =& $cs->getRef("value");

		//$mInfo =& $this->getRef("missileInfo");
		$nextLevInfo = $mInfo[$level+1];
		$isMax=false;

		if($level>=0)$currentLevInfo = $mInfo[$level];
		else $currentLevInfo=array(0,0);
		
		if($level-1>=0)$prevLevInfo = $mInfo[$level-1];
		else $prevLevInfo=array(0,0);

		if($level>=count($mInfo)-1){
			$level = count($mInfo)-1;
			$isMax=true;
		...
		$r["prevPrice"]=$prevLevInfo[1];
		$r["currentPrice"]=$currentLevInfo[1];
    	$r["power"]=$currentLevInfo[0];
    	$r["nextPower"]=$nextLevInfo[0];
    	$r["prevPower"]= $prevLevInfo[0];
    	$r["isMaxLevel"]=$isMax;
    	$r["maxLevel"]=count($mInfo)-1;
...
class Exchange extends DBTable2{
	...
	static public function getCategory(){
		...
		$list["m"]["title"]="결제(m)";
		$list["m"]["suffix"]="원";
		$list["m"]["placeholder"]="가격";
		
		$list["g"]["title"]="골드(g)";
		$list["g"]["suffix"]="개";
		$list["g"]["placeholder"]="갯수";

		$list["r"]["title"]="젬(r)";
		$list["r"]["suffix"]="개";
		$list["r"]["placeholder"]="갯수";

		$list["pr"]["title"]="유료젬(pr)";
		$list["pr"]["suffix"]="개";
		$list["pr"]["placeholder"]="갯수";

		$list["fr"]["title"]="무료젬(fr)";
		$list["fr"]["suffix"]="개";
		$list["fr"]["placeholder"]="갯수";

		$list["h"]["title"]="하트(h)";
		$list["h"]["suffix"]="개";
		$list["h"]["placeholder"]="갯수";

		$list["i6"]["title"]="더블아이템(i6)";
		$list["i6"]["suffix"]="개";
		$list["i6"]["placeholder"]="갯수";

		$list["i9"]["title"]="신발아이템(i9)";
		$list["i9"]["suffix"]="개";
		$list["i9"]["placeholder"]="갯수";

		$list["i11"]["title"]="자석아이템(i11)";
		$list["i11"]["suffix"]="개";
		$list["i11"]["placeholder"]="갯수";

		$list["p1"]["title"]="부활석(p1)";
		$list["p1"]["suffix"]="개";
		$list["p1"]["placeholder"]="갯수";

		$list["p2"]["title"]="맵가챠권(p2)";
		$list["p2"]["suffix"]="개";
		$list["p2"]["placeholder"]="갯수";

		$list["p3"]["title"]="미사일업글권(p3)";
		$list["p3"]["suffix"]="개";
		$list["p3"]["placeholder"]="갯수";

		$list["p4"]["title"]="아이템뽑기권(p4)";
		$list["p4"]["suffix"]="개";
		$list["p4"]["placeholder"]="갯수";

		$list["p5"]["title"]="99프로뽑기권(p5)";
		$list["p5"]["suffix"]="개";
		$list["p5"]["placeholder"]="갯수";

		$list["p6"]["title"]="생명의돌(p6)";
		$list["p6"]["suffix"]="개";
		$list["p6"]["placeholder"]="갯수";

		$list["p7"]["title"]="일반뽑기권(p7)";
		$list["p7"]["suffix"]="개";
		$list["p7"]["placeholder"]="갯수";

		$list["p8"]["title"]="고급뽑기권(p8)";
		$list["p8"]["suffix"]="개";
		$list["p8"]["placeholder"]="갯수";

		$list["msg"]["title"]="메세지(msg)";
		$list["msg"]["suffix"]="";
		$list["msg"]["placeholder"]="메세지(재화선물하기 전용)";

		$list["cd"]["title"]="카드추가(cd)";
		$list["cd"]["suffix"]="번";
		$list["cd"]["placeholder"]="카드번호";

		$list["cdm"]["title"]="카드제거(cdm)";
		$list["cdm"]["suffix"]="번";
		$list["cdm"]["placeholder"]="카드번호";

		$list["pc"]["title"]="피스(pc)";
		$list["pc"]["suffix"]="번";
		$list["pc"]["placeholder"]="피스번호";

		$list["pz"]["title"]="퍼즐(pz)";
		$list["pz"]["suffix"]="번";
		$list["pz"]["placeholder"]="퍼즐번호";

		$list["cu"]["title"]="미사일업글(cu)";
		$list["cu"]["suffix"]="번";
		$list["cu"]["placeholder"]="캐릭번호";

		$list["cp"]["title"]="캐릭터(cp)";
		$list["cp"]["suffix"]="번";
		$list["cp"]["placeholder"]="캐릭번호";

		$list["gncd"]["title"]="노말카드랜덤(gncd)";
		$list["gncd"]["suffix"]="등급";
		$list["gncd"]["placeholder"]="등급번호, 0이면 전체랜덤";

		$list["dhcd"]["title"]="미보유 노말카드랜덤(dhcd)";
		$list["dhcd"]["suffix"]="등급";
		$list["dhcd"]["placeholder"]="등급번호, 0이면 전체랜덤";
...
class EndlessRank extends DBTable2{
	static public function construct(){
		self::setPrimarykey("no",true);
		self::setDBGroup("log");
		self::setDBTable("EndlessRank_".TimeManager::getCurrentWeekNoForRank());
		self::setShardKey("memberID");
...
class EndlessPlayList extends DBTable2{
	...
	public function getPlayDataByRandom($memberID,$lvl=1,$limit=1,$fieldlist="*"){
		...
		while(count($result)<$limit){
			$lvl--;
			//LogManager::addLog("select ".$fieldlist." from `".EndlessPlayList::getDBTable()."` where memberID in (SELECT DISTINCT memberID FROM `".EndlessPlayList::getDBTable()."`) and memberID <> ".$memberID." and victory>=".$lvl." ORDER BY RAND() limit ".$limit);
			$query = EndlessPlayList::getQueryResult("select ".$fieldlist." from `".EndlessPlayList::getDBTable()."` where memberID in (SELECT DISTINCT memberID FROM `".EndlessPlayList::getDBTable()."`) and memberID <> ".$memberID." and victory>=".$lvl." ORDER BY RAND() limit ".$limit);
			if($query){
				while($rData = mysql_fetch_assoc($query)){
					if($check)unset($rData["playData"]);
					$result[]=$rData;
...
class Gacha extends DBTable2{
	static public function construct(){
		self::setPrimarykey("no",true);
		self::setDBGroup("main");
		self::setDBTable("aGachaTable");
	}
	...
	public function __construct($no=null,$level=null,$isPremium=false){
		...
		if($level){
			...
			//ORDER BY rand()*100+weight DESC LIMIT 1;
			$q_where = "level=".$level." ".$pre." and showProb>0 order by rand()*100+showProb desc limit 1";
			parent::load($q_where);
		}
