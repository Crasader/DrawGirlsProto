...
for($c=0;$c<count($param);$c++){
	...
	if($a == "getstageinfo"){
		$stageInfo =  mysql_fetch_array(mysql_query("select * from ".DBManager::getMT("stage")." where no=$p[no]"));
		...
		if($p[version]<$stageInfo[version] || $p[version]==-1){
			...
			for($i=0;$i<count($stageInfo[cards]);$i++){
				$cardInfo = mysql_fetch_array(mysql_query("select * from ".DBManager::getMT("card")." where no=".$stageInfo[cards][$i]));
...
"stageList":[
		...
		*/
		...
		//where theme=$theme
		$query = mysql_query("select no,thumbnail from ".DBManager::getMT("stage"));
