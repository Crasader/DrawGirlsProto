 ...
 *		{
 *			for ( HSPRealtionResult* result in relationResults )
 *			{
 *				NSLog(@"Result = %@", result);
 *			}
 *		}
 ...
 *
 * @code
 *	[HSPAmigo deleteMatePropsalWithMemberNos:memberNos completionHandler:^(NSArray *relationResults, HSPError *error) {
 *		if ( [error isSuccess] == YES )
 *		{
 *			for ( HSPRealtionResult* result in relationResults )
 *			{
 ...
 * @serviceDomain ALL
 */
+ (void)deleteMatePropsalWithMemberNos:(NSArray*)memberNos
					 completionHandler:(void (^)(NSArray* relationResults, HSPError* error))completionHandler;
 ...
 * 	if ( [error isSuccess] == YES )
 * 	{
 * 		for ( HSPRelationResult* result in relationResults )
 *		{
 *			NSLog(@"Result : %@", result);
 *		}
 * 	} 
