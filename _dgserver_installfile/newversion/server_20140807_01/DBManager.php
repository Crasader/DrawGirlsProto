...
class Puzzle extends DBTable2{
	...
	public static function loadWithLQTable($p){
		...
		$eventEditor = json_decode('{"type":"select","element":["비공개","일반","이벤트"],"value":[-1,0,1]}',true);
		...
		if($p["name"]=="openCondition"){
			$data["head"][]=array("title"=>"고유번호","field"=>"no","viewer"=>$textViewer,"primary");
			$data["head"][]=array("title"=>"순서","field"=>"order","viewer"=>$textViewer,"editor"=>$intEditor);
			$data["head"][]=array("title"=>"공개여부","field"=>"isEvent","viewer"=>$eventEditor,"editor"=>$eventEditor);
			$data["head"][]='{"title":"타이틀","field":"title","viewer":{"type":"languageViewer"},"editor":{"type":"languageEditor"}}';
			$data["head"][]=array("title"=>"이미지","field"=>"original","viewer"=>$imgViewer,"editor"=>$imgEditor);
			$data["head"][]=array("title"=>"오픈조건","field"=>"condition","viewer"=>json_decode('{"type":"puzzleOpenConditionViewer"}',true),"editor"=>$conditionEditor);
			$data["head"][]=array("title"=>"보상카드","field"=>"clearReward","viewer"=>json_decode('{"type":"puzzleCardRewradViewer"}',true),"editor"=>json_decode('{"type":"dictionary","element":[{"type":"text","field":"normal"},{"type":"text","field":"perfect"}]}',true));
			...
			$data["head"][]=array("field"=>"isEvent","viewer"=>$eventEditor,"editor"=>$eventEditor);
...
class Card extends DBTable2{
	...
	public static function loadWithLQTableForCardInfo($p){
		...
		$data["head"][]=array("title"=>"스테이지","field"=>"piece","viewer"=>json_decode('{"type":"text"}',true),'always');
		...
		$data["head"][]=array("title"=>"사운드","field"=>"sound","viewer"=>json_decode('{"type":"arrayViewer"}',true),"editor"=>json_decode('{"type":"array","element":{"type":"text"}}',true));
